...
def ViewPatientReport(request):
    if request.method == 'GET':
        ...
        output+='<th><font size="" color="black">Permitted Admin</th>'  # Changed 'Doctor' to 'Admin'
        ...
        with con:
            cur = con.cursor()
            cur.execute("select * from patient_data where permit_admin='"+uname+"'")  # Changed 'permit_doctor' to 'permit_admin'
            rows = cur.fetchall()
            output+='<tr>'
            for row in rows:
        ...
        context= {'data':output}
        return render(request, 'AdminScreen.html', context)  # Changed 'DoctorScreen' to 'AdminScreen'
...
def AdminLogin(request):  # Changed 'DoctorLogin' to 'AdminLogin'
    if request.method == 'GET':
       return render(request, 'AdminLogin.html', {})  # Changed 'DoctorLogin.html' to 'AdminLogin.html'
...
def AdminLoginAction(request):  # Changed 'DoctorLoginAction' to 'AdminLoginAction'
    if request.method == 'POST':
        global uname, pin
        username = request.POST.get('t1', False)
        password = request.POST.get('t2', False)
        ...
        if index == 1:
            ...
            context= {'data':'login failed'}
            return render(request, 'AdminLogin.html', context)  # Changed 'DoctorLogin.html' to 'AdminLogin.html'
